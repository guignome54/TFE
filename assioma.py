...
BATTERY_LEVEL_MEASURE = bluetooth.UUID(0x2A19)
...
class AssiomaBLEClient:
        ...
        try:
            if event == _IRQ_SCAN_RESULT:
                ...
                if uuid == CPM_SERVICE_UUID:
                    print("Service Cycling Power trouvé")
                    self._service_start_handle = start_handle
                    self._service_end_handle = end_handle
                ...
                # Vérifier si nous avons besoin de découvrir les caractéristiques de batterie
                if hasattr(self, '_battery_service_start_handle') and hasattr(self, '_battery_service_end_handle') and not self.battery_level_handle:
                    time.sleep_ms(500)  # Petit délai entre les découvertes
                    try:
                        self._discovering_chars = True
                        self.ble.gattc_discover_characteristics(
                            self.conn_handle, self._battery_service_start_handle, self._battery_service_end_handle
                        )
                        return  # Sortir ici pour ne pas configurer les notifications maintenant
                    except Exception as e:
                        print(f"Erreur découverte caractéristiques batterie: {e}")
                        self._discovering_chars = False
                
                # Si nous ne sommes pas en train de découvrir de nouvelles caractéristiques,
                # nous pouvons configurer les notifications
                time.sleep_ms(500)
    ...
    def _discover_characteristics(self):
        """Découvre les caractéristiques des services"""
        if not self.conn_handle or self._discovering_chars:
            return
            
        try:
            self._discovering_chars = True
            # Découvrir d'abord les caractéristiques du service de puissance
            if hasattr(self, '_cpm_service_start_handle') and hasattr(self, '_cpm_service_end_handle'):
                self.ble.gattc_discover_characteristics(
                    self.conn_handle, self._cpm_service_start_handle, self._cpm_service_end_handle
                )
                # Attendre que cette opération se termine avant de découvrir les caractéristiques de batterie
                # La suite sera gérée par le callback _IRQ_GATTC_CHARACTERISTIC_DONE
            
            # Ne pas découvrir les caractéristiques du service de batterie ici
            # Cela sera fait dans le callback _IRQ_GATTC_CHARACTERISTIC_DONE
        except Exception as e:
